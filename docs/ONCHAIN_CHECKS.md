...
本文件记录链上核对的关键地址与参数，来源于 `script/ChainChecks.s.sol`。
...
│ Cooldown (sec)    │ 172800                                        │
├───────────────────┼──────────────────────────────────────────────┤
│ INIT_CODE_HASH    │ 0x00fb7f630766e6a796048ea87d01acd3068e8ff67d07 │
│                   │ 8148a3fa3f4a84f69bd5                           │
└───────────────────┴──────────────────────────────────────────────┘
```
...
Fee is encoded in the core swap formula and the periphery library:
...
- `lastMintedTimestamp(address)` 可能不在当前 diamond ABI 中, 脚本返回 0.

- INIT_CODE_HASH 来源于 factory on-chain call: `INIT_CODE_PAIR_HASH()`.
